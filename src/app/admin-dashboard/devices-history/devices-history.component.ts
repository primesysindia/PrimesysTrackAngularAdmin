...
import { BatteryInfoModuleComponent } from '../battery-info-module/battery-info-module.component';
...
@Component({
  ...
  styleUrls: ['./devices-history.component.css'],
  animations: [
    trigger('slideInOut', [
      transition(':enter', [
...
export class DevicesHistoryComponent implements OnInit {
  ...
  getHistory() {
    ...
    let endDateTime = Math.floor(this.f.toTime.value/1000)
      ...
      // this.showB(imei, startDateTime,endDateTime)
      if(chkExpiry>=0){
            ...
            if(data.historyDataSize == 0){
              ...
              if(this.markers.length>1){
                this.speedChange = 8;
                  ...
                  this.beatService.GetRDPSdata(this.markers[idx].lat ,this.markers[idx].lng)
                  .subscribe(data=>{
                    this.rdps = data;
  ...
  getDevicesList(pId){
    ...
    showBatteryChart: boolean;
    ShowBatteryInfo() 
    {
      let imei = this.selectedDevice.imei_no;
      let startDateTime = Math.floor(this.f.fromTime.value/1000)
      let endDateTime = Math.floor(this.f.toTime.value/1000)

      // console.log("working..", imei)
      this.showChart = false;
      ...
      dialogConfig.data = {
          width: '400px',
          imei: imei,
          startDTime: startDateTime,
          endDTime: endDateTime
      };
      let dialogRef = this.dialog.open(BatteryInfoModuleComponent, dialogConfig)
